...
FROM
    ...
    LEFT OUTER JOIN us_cases_all as yesterday ON TO_DATE(yesterday.date, 'YYYY-MM-DD') = TO_DATE(today.date, 'YYYY-MM-DD') - interval '1 day'
    LEFT OUTER JOIN us_cases_all as twoweeks ON TO_DATE(twoweeks.date, 'YYYY-MM-DD') = TO_DATE(today.date, 'YYYY-MM-DD') - interval '14 day'
where
    today.date = (
        select
            max(date)

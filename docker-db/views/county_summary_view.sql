DROP MATERIALIZED VIEW if exists county_summary_view;CREATE MATERIALIZED VIEW county_summary_view AS
SELECT
...
FROM
    ...
    LEFT JOIN (
        ...
        FROM
            county_cases_all today
            JOIN county_cases_all yesterday ON to_date(yesterday.date, 'YYYY-MM-DD' :: text) = (
        ...
        WHERE
            today.date = (
                (
                    SELECT
                        max(county_cases_all.date) AS max
                    FROM
                        county_cases_all
                )
            )
    ) cases ON county_meta.county_fips_code = cases.county_fips_code;
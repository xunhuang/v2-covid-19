drop TABLE if exists county_meta;
...
from fips_code_county as county
left outer join state_meta as state
on county.state_fips_code = state.state_fips_code
left outer join msa_counties as msa_counties
on msa_counties.county_fips_code = county.county_fips_code
left outer join msa as msa
on msa.msa_id = msa_counties.msa_id
left outer join county_population as pop
on county.county_fips_code = pop.county_fips_code;